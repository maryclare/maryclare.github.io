...
Making a matrix is a bit different from making a vector. We **cannot** grow a matrix - we need to start from a matrix initialized to have the desired dimensions.
...
```{r, eval = TRUE, echo = TRUE}
colnames(A) <- c("Sad", "Happy")
row.names(A) <- c("Red", "Hope", "Wavy")
```
