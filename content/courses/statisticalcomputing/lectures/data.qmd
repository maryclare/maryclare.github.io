...
When we first enter `data(rivers)`, `R` creates one or more objects that have value `<Promise>`.  
...
For example, we can create a new variable in the `stackloss` data frame called `variable` that contains the order that each observation appears in.
