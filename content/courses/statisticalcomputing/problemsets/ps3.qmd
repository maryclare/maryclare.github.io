...
(a) Create a matrix with 9 rows and 100 columns called `Y` with each column defined as `x + sample((-3):3, 9, replace = TRUE)`. Print the code you use to create this matrix to the rendered `.pdf`.
...
(c) In class, we smoothed `y` by averaging observations with their nearest neighbors in each direction (`z` with `z[1] = (y[1] + y[2])/2`, `z[2] = (y[1] + y[2] + y[3])/3`, etc.) and then again by averaging observations with their $k$ nearest neighbors in each direction  (for $k = 2$, we have `w` with `w[1] = (y[1] + y[2] + y[3])/3`, `w[2] = (y[1] + y[2] + y[3] + y[4])/4`, `w[3] = (y[1] + y[2] + y[3] + y[4] + y[5])/5`, etc.). Create matrices `Z` and `W` of the same dimensions as `Y`, but with columns that contain smoothed columns of `Y` obtained by averaging observations with their nearest neighbors in each direction (for `Z`) and their nearest two neighbors in either direction (for `W`). You can use the `smoothing` function defined in `smoothing.R`. Print the code you use to create these matrices to the rendered `.pdf`. 

(d) Create vectors `ymean`, `zmean`, and `wmean` which contain the average of each row of `Y`, `Z`, and `W`, respectively.  Print the code you use to create these vectors  to the rendered `.pdf`.

(e) Create vectors `ysd`, `zsd`, and `wsd` which contain the standard deviation of each row of `Y`, `Z`, and `W`, respectively.  Print the code you use to create these vectors to the rendered `.pdf`.
